  ...
  <Navbar>
    <div class="w-full h-[95vh]">
      <div class="container">
        ...
        <div
          v-if="selectedRegion && filteredResults.length === 0"
          class="no-results"
        >
...
async function fetchRegions() {
  try {
    const response = await fetch("http://result.test/api/region");
...
async function handleChange() {
  try {
    const response = await fetch(
      `http://result.test/api/resultlist?region_id=${selectedRegion.value}`
    );
